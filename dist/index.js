...
var insertProjectCommentSchema = createInsertSchema(projectComments).pick({
  projectId: true,
  userId: true,
  userName: true,
  content: true
});
var projectParticipants = pgTable("project_participants", {
  id: serial("id").primaryKey(),
  projectId: integer("project_id").notNull(),
  userId: integer("user_id").notNull(),
  userName: text("user_name"),
...
var insertProjectParticipantSchema = createInsertSchema(projectParticipants).pick({
  projectId: true,
  userId: true,
  userName: true,
  role: true
...
var SYSTEM_PROMPT = `You are Curio, a friendly and encouraging AI science learning companion for Bangladeshi students. Your purpose is to help students learn science concepts through engaging conversations.
...
async function generateCurioResponse(messages) {
  if (!apiKey) {
  ...
  try {
    ...
    console.log(`Gemini response generated - Length: ${text2.length} chars`);
    return text2;
  } catch (error) {
...
async function registerRoutes(app2) {
  ...
  const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
      if (file.mimetype === "application/pdf" || file.mimetype === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" || file.mimetype === "application/msword" || file.mimetype === "text/plain" || file.mimetype === "image/jpeg" || file.mimetype === "image/png") {
        cb(null, true);
      } else {
        cb(null, false);
...
try {
  if (fs2.existsSync("./.env")) {
    const envFile = fs2.readFileSync("./.env", "utf8");
...
var attachedAssetsPath = path3.join(process.cwd(), "attached_assets");
var resourcesPath = path3.join(process.cwd(), "public", "resources");
app.use("/attached_assets", express2.static(attachedAssetsPath));
app.use("/resources", express2.static(resourcesPath));
